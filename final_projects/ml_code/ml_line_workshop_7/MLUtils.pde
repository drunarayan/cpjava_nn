...
void print_results(Matrix out, int rec) {
  double[][] res1 = out.getArray();
  double nn_prediction_slope = res1[0][0];
  double nn_prediction_offset = res1[1][0];
  double[][] res2 = target.getArray();
  double nn_target_slope = res2[0][0];
  double nn_target_offset = res2[1][0];
  System.out.println(System.out.format("For input record %d slope %.3f offset %.3f neural network predicts slope %.3f offset %.3f\n",
    rec, nn_target_slope, nn_target_offset, nn_prediction_slope, nn_prediction_offset));
}
...
void print_stats(int rec) {
  ...
  System.out.println(System.out.format("For all %d input records neural network prediction accuracy is %.1f percent \n", rec, ((double)success_count/(error_count+success_count)*100.0)));
}
...
void calc_stats(Matrix out) {
  ...
  double nn_target_slope = res2[0][0];
  double nn_target_offset = res2[1][0];
  if (Math.abs(nn_target_slope - nn_prediction_slope) < 0.1 && Math.abs(nn_target_offset - nn_prediction_offset) < 0.1) {
    success_count++;
  } else {
    error_count++;
  }
...
void create_input_target_rec(String curr) {
  ...
  for (int i = 0; i < atgt.length; i++) {
    atgt[i][0] = linp.get(i);  // 0th, 1st, 2nd elements of record
  }
  ...
  for (int i = 0; i < ainp.length; i++) {
    ainp[i][0] = linp.get(i+3); // 3rd of 4th element of the record
  }
